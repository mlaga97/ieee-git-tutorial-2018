...
void loop() {
  for (int thisNote = 0; thisNote < 14; thisNote++) { 
    duration = 600;
    if (thisNote == 6 || thisNote == 13 || thisNote == 20 || thisNote == 27 || thisNote == 34 || thisNote == 41) {
      duration = 1200;
    }
    // Turn on onboard LED and play current note
    tone(PASSIVE_BUZZER, melody[thisNote], duration);
    digitalWrite(LED_BUILTIN, HIGH);   
    delay(duration);
    // Turn off onboard LED and play next note after 100 milliseconds
    digitalWrite(LED_BUILTIN, LOW);
    delay(100);
  }
  // restart song after two seconds 
  delay(2000);
}
